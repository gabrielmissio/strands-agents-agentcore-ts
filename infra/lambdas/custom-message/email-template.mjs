 ...
 * password) message and the self sign-up confirmation code.
 *
 * Split from `index.mjs` so that file stays the trigger's plumbing (which message type, where the
 * app URL comes from, failure handling) and this one is the content. Still plain `.mjs` with no
 * imports — the whole asset must remain buildable by nothing.
 *
...
/** The card/table chrome both emails share. */
...
/**
 * Renders the admin-invite (temporary password) email.
 *
 * `{username}` and `{####}` are Cognito's own placeholders — it substitutes the username and the
 * temporary password when it sends. An invite that loses `{####}` goes out with no password.
 */
export function renderInviteEmail({ appName, appUrl } = {}) {
  const app = escapeHtml(appName || 'the app')

  const rows = [
    heading(app),
    `<tr>${cell('Hello {username},', 'padding-bottom:12px;')}</tr>`,
    `<tr>${cell(
      'An administrator created an account for you. Use this temporary password to sign in.',
      'padding-bottom:20px;',
    )}</tr>`,
    codeBlock('Temporary password', '{####}'),
    `<tr>${cell(
      `<span style="color:${MUTED};">You will be asked to choose your own password the first time you sign in.</span>`,
      'font-size:14px;padding-bottom:24px;',
    )}</tr>`,
    ctaRow('Sign in at', appUrl),
    footer(
      `You received this message because an account was created for you at ${app}. If you were not expecting it, you can ignore this email.`,
    ),
  ]
...
export function buildInviteMessage(appName, appUrl) {
  return {
    subject: `Your ${appName || 'the app'} access`,
    body: renderInviteEmail({ appName, appUrl }),
  }
}

/**
 * Renders the self sign-up confirmation email.
 *
 * `{####}` is Cognito's placeholder for the verification code. This trigger source carries no
 * username placeholder, so the greeting stays generic.
 */
export function renderVerificationEmail({ appName, appUrl } = {}) {
  const app = escapeHtml(appName || 'the app')

  const rows = [
    heading(app),
    `<tr>${cell('Confirm your email to finish creating your account.', 'padding-bottom:20px;')}</tr>`,
    codeBlock('Verification code', '{####}'),
    `<tr>${cell(
      `<span style="color:${MUTED};">Enter this code where you signed up. It expires shortly, so use it soon.</span>`,
      'font-size:14px;padding-bottom:24px;',
    )}</tr>`,
    ctaRow('Sign in at', appUrl),
    footer(
      `You received this message because someone signed up for ${app} with this email address. If that was not you, you can ignore this email.`,
    ),
  ]
...
export function buildVerificationMessage(appName, appUrl) {
  return {
    subject: `Confirm your ${appName || 'the app'} account`,
    body: renderVerificationEmail({ appName, appUrl }),
  }
}
