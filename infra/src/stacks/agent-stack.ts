...
export interface AgentStackProps extends cdk.StackProps {
  ...
  /**
   * The Cognito identity pool's authenticated role, granted `InvokeAgentRuntime` scoped to this
   * stack's runtime ARN once it exists. Kept out of the auth stack: that role is assumable by every
   * signed-in browser, so its permissions must never be broader than the one resource each caller
   * actually needs.
   */
...
export class AgentStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: AgentStackProps) {
    ...
    // Grant every signed-in browser's role permission to invoke *this* runtime, and nothing else.
    // This is the only place that permission should be attached — see AgentStackProps.invokerRole.
    //
    // A standalone `Policy` scoped to *this* stack, rather than `invokerRole.addToPrincipalPolicy()`:
    // that would attach the statement to the role's own default policy, which is a child of the role
    // and therefore lives in the auth stack that defines it. Since that policy's resource is this
    // runtime's ARN, the auth stack would need to import a value from this stack — and this stack
    // already depends on the auth stack (for the Cognito discovery URL above), so that would be a
    // synth-time dependency cycle. Scoping the `Policy` construct here instead keeps the only new
    // cross-stack reference pointing the same direction as the existing one.
    if (invokerRole) {
      new iam.Policy(this, 'AuthenticatedRoleInvokePolicy', {
        statements: [
          new iam.PolicyStatement({
            effect: iam.Effect.ALLOW,
            actions: ['bedrock-agentcore:InvokeAgentRuntime'],
            resources: [this.runtimeArn, `${this.runtimeArn}/runtime-endpoint/*`],
          }),
        ],
        roles: [invokerRole],
      })
