...
type UsersResult = { users?: AdminUser[]; errorMessage?: string }
...
async function fetchUsers(): Promise<UsersResult> {
  try {
    ...
    return { errorMessage: err instanceof Error ? err.message : 'Could not load users' }
  }
}

function statusLabel(user: AdminUser): string {
  if (!user.enabled) return 'Disabled'
  if (user.status === PENDING_STATUS) return 'Invite pending'
  if (user.status === 'CONFIRMED') return 'Active'
...
export function AdminPanel({ userEmail, onSignOut, onBack }: AdminPanelProps) {
  ...
  const applyResult = useCallback((result: UsersResult) => {
    ...
    setLoadError(result.users ? '' : (result.errorMessage ?? 'Could not load users'))
    setLoading(false)
  }, [])
      ...
      <header className="relative z-10 border-b-[3px] border-cave/80 bg-secondary/60 backdrop-blur">
        <div className="mx-auto flex max-w-4xl items-center gap-3 px-4 py-3">
          ...
          <div className="leading-tight">
            <h1 className="font-display text-lg uppercase text-cave sm:text-xl">Admin</h1>
        ...
        <div className="mx-auto flex max-w-3xl flex-col gap-5 px-4 py-6">
          ...
          <section className="rounded-2xl border-[3px] border-cave bg-card p-4 shadow-[var(--shadow-stone)]">
            ...
            <p className="mt-3 text-[11px] font-semibold text-muted-foreground">
              Cognito emails the address a temporary password. They set their own on first sign-in.
            </p>
          </section>
          ...
          <section className="rounded-2xl border-[3px] border-cave bg-card p-4 shadow-[var(--shadow-stone)]">
            <div className="mb-3 flex items-center gap-2">
              <h2 className="font-display text-sm uppercase text-cave">{users.length} members</h2>
              <button
                type="button"
